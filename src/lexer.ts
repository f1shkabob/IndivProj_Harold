...
export interface TNum { tag: 'num', value: number }
const tnum = (value: number): Tok => ({ tag: 'num', value })

export interface TIdent { tag: 'ident', value: string }
const tident = (value: string): Tok => ({ tag: 'ident', value })

export interface TLParen { tag: '(' }
const tlparen: Tok = ({ tag: '(' })

export interface TRParen { tag: ')' }
const trparen: Tok = ({ tag: ')' })

export interface TTrue { tag: 'true' }
const ttrue: Tok = ({ tag: 'true' })

export interface TFalse { tag: 'false' }
const tfalse: Tok = ({ tag: 'false' })

export interface TUnit { tag: 'unit' }
const tunit: Tok = ({ tag: 'unit' })

export interface TPair { tag: 'pair' }
const tpair: Tok = ({ tag: 'pair' })

/** The type of tokens generated by the lexer. */
export type Tok = TNum | TIdent | TLParen | TRParen | TTrue | TFalse | TUnit | TPair
